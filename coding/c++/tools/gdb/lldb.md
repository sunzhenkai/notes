...
```shell
process handle -p true -s false -n true SIGRTMIN
process handle -p true -s false -n true SIGINT
process handle -p true -s false -n true SIGUSR1
process handle -p true -s false -n true SIGUSR2
process handle -p true -s false -n true SIGRTMIN
process handle -p true -s false -n true SIGRTMIN+1
# 或添加在配置文件 ~/.lldbinit 中
```
